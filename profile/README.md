...
| [pgd](https://github.com/power-grid-ml/pgd) | `pip install power-grid-dash` | `pgd` | a FastAPI service + web UI over simulation, state estimation and live measurements | planned: 2026-11 |
| [power-grid-suite](https://github.com/power-grid-ml/power-grid-suite) | `pip install power-grid-suite` | — | everything above in one tested combination; for developers, every package editable in one environment (submodules + pixi) and the SLURM cluster jobs — **start here while nothing is on PyPI** | planned: 2027-03 |
